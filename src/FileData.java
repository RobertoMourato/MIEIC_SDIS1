...
public class FileData {
    ...
    private static ArrayList<Chunk> generateChunksFromFile(File file, String fileId) {
        ...
        try (FileInputStream fis = new FileInputStream(file)) {
            ...
            while ((bytesRead = fis.read(curChunk)) > 0){
                chunks.add(new Chunk(fileId, chunkNumber++, Arrays.copyOf(curChunk, bytesRead)));
            }

            if (file.length() % MAX_SIZE_CHUNK == 0){
                chunks.add(new Chunk(fileId, chunkNumber, null));
            }
