#! /usr/bin/bash

# Script for running the test app
# To be run at the root of the compiled tree
# No jar files used
# Assumes that TestApp is the main class 
#  and that it belongs to the test package
# Modify as appropriate, so that it can be run 
#  from the root of the compiled tree

# Check number input arguments
argc=$#

if (( argc < 2 )) 
then
	echo "Usage: $0 <host>/<peer_access_point> BACKUP|RESTORE|DELETE|RECLAIM|STATE [<opnd_1> [<optnd_2]]"
	exit 1
fi

# Assign input arguments to nicely named variables

pap=$1
oper=$2

# Validate remaining arguments 

case $oper in
BACKUP)
	if(( argc != 4 )) 
	then
		echo "Usage: $0 <host>/<peer_access_point> BACKUP <filename> <rep degree>"
		exit 1
	fi
	opernd_1=$3
	rep_deg=$4
	;;
RESTORE)
	if(( argc != 3 ))
	then
		echo "Usage: $0 <host>/<peer_access_point> RESTORE <filename>"
	fi
	opernd_1=$3
	rep_deg=""
	;;
DELETE)
	if(( argc != 3 ))
	then
		echo "Usage: $0 <host>/<peer_access_point> DELETE <filename>"
		exit 1
	fi
	opernd_1=$3
	rep_deg=""
	;;
RECLAIM)
	if(( argc != 3 ))
	then
		echo "Usage: $0 <host>/<peer_access_point> RECLAIM <max space>"
		exit 1
	fi
	opernd_1=$3
	rep_deg=""
	;;
STATE)
	if(( argc != 2 ))
	then
		echo "Usage: $0 <host>/<peer_access_point> STATE"
		exit 1
	fi
	opernd_1=""
	rep_deg=""
	;;
*)
	echo "Usage: $0 <host>/<peer_access_point> BACKUP|RESTORE|DELETE|RECLAIM|STATE [<opnd_1> [<optnd_2]]"
	exit 1
	;;
esac

# Execute the program
# Should not need to change anything but the class and its package, unless you use any jar file

# echo "java test.TestApp ${pap} ${oper} ${opernd_1} ${rep_deg}"

java Application ${pap} ${oper} ${opernd_1} ${rep_deg}
